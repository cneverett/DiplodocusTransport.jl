...
# tuple of vectors, each vector representing [1, 2, 3, 4] particles in the interactions to include in the simulation i.e. [[interaction1], [interaction2], ...]
    ...
    interaction_list_Binary::Vector{Vector{String}} = [["Sph", "Sph", "Sph", "Sph"],];
    interaction_list_Emi::Vector{Vector{String}} = [];

# Collate lists into a single struct to be passed to the solver
...
# Load interaction matrices
    ...
    #BES.LoadMatrices_Binary(BES.Matrices_BinaryInteraction,DataDirectory,Lists;mode=mode);
...
# Set initial conditions
    ...
    f1D0_Sph = BES.Initial_Constant(Lists,"Sph",41,41,4,5,1f0 #=1cm^{-3}=#;mode=mode);

# Solver Setup and Time Stepping

    f1D0 = ArrayPartition(f1D0_Sph,);
...
# Plotting
    ...
    BES.AllPlots_Ani(sol,num_species,name_list,pr_list,ur_list,dp_list,du_list,meanp_list,meanu_list,numInit_list,engInit_list,tempInit_list,mass_list,p_num_list,u_num_list,"HardSphere.mp4",24;istart=1,istop=421,iframe=nothing,initial=true,thermal=true)
