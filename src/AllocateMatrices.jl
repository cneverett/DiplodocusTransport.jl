function Allocate_A_Binary(Lists::ListStruct)
    ...
    A_Binary::Matrix{Float32} = zeros(Float32,m,n)
    #A_Binary::Array{Float32} = zeros(Float32,n,n,n)
...
function Allocate_J_Binary(Lists::ListStruct)
    ...
    J_Binary::Matrix{Float32} = zeros(Float32,n,n)

    size_J_Binary = sizeof(J_Binary)

    if size_J_Binary > 1e9
        println("A_Binary is approx. $(size_J_Binary/1e9) GB in memory")
    elseif size_J_Binary > 1e6
        println("A_Binary is approx. $(size_J_Binary/1e6) MB in memory")
    elseif size_J_Binary > 1e3
        println("A_Binary is approx. $(size_J_Binary/1e3) KB in memory")
    else
        println("A_Binary is approx. $size_J_Binary bytes in memory")
    end
...
function Fill_A_Binary!(A_Binary::Array{Float32},interaction::Vector{String},Lists::ListStruct;SMatrix3=nothing,SMatrix4=nothing,TMatrix1=nothing,TMatrix2=nothing)
    ...
    # redefine each matrix as nothing or 0 and then call garbage collector to free memory 
